...
objective-scoped round for ExecutionAgent, or you stop/pause/replan. ExecutionAgent decides the
concrete tool sequence inside the round.
...
- record_attack_step: {"tool":"record_attack_step","arguments":{"target_ref":"kg/ag node id or null","status":"succeeded|partial|failed|blocked|needs_replan","summary":"brief summary","evidence_refs":[]}}
...
3. When eligible_for_stop is false, map success_condition_progress.missing to a bounded objective:
   discover assets/services for missing recon, identify vulnerability candidates for missing analysis,
   gain access/session for missing exploitation, establish restricted-zone reachability for missing
   pivot/lateral, capture proof for missing goal evidence.
...
6. An exploitation objective means authorized real exploitation inside scope, including shell/session/
   command execution where policy allows. Do not downgrade it to safe validation.
   When the ToolCatalog includes metasploit_exec and the missing condition is exploit success or
   session, make metasploit_exec the preferred exploit tool via allowed_tools/tool_hints.
...
9. Round granularity is controlled by objective width + max_tools, not by chaining phases/steps.
