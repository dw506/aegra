...
TEMPLATES: dict[str, ActionTemplate] = {
    ...
    "scan_ports": ActionTemplate(
        action_type="scan_ports",
        task_type=TaskType.PORT_SCAN,
        label="Scan ports",
        resource_key_fields=("host_id", "target_host", "target_cidr"),
        default_bindings={"tool_hint": "nmap"},
    ),
    ...
    "fingerprint_internal_service": ActionTemplate(
        action_type="fingerprint_internal_service",
        task_type=TaskType.INTERNAL_SERVICE_FINGERPRINT,
        label="Fingerprint internal service",
        resource_key_fields=("service_id", "host_id", "route_id", "session_id"),
        default_bindings={"tool_hint": "httpx"},
    ),
    "discover_web_paths": ActionTemplate(
        action_type="discover_web_paths",
        task_type=TaskType.WEB_DISCOVERY,
        label="Discover web paths",
        resource_key_fields=("service_id", "host_id", "target_url"),
        default_bindings={"tool_hint": "web_discovery"},
    ),
    ...
    "validate_lateral_reachability": ActionTemplate(
        action_type="validate_lateral_reachability",
        task_type=TaskType.LATERAL_REACHABILITY_VALIDATION,
        label="Validate lateral reachability",
        estimated_risk=0.4,
        estimated_noise=0.2,
        approval_required=True,
        resource_key_fields=("source_host_id", "target_host_id", "route_id", "session_id"),
        default_bindings={"tool_hint": "pivot_probe"},
    ),
    "validate_credential": ActionTemplate(
        action_type="validate_credential",
        task_type=TaskType.CREDENTIAL_VALIDATION,
        label="Validate credential",
        estimated_risk=0.35,
        estimated_noise=0.15,
        resource_key_fields=("credential_id", "host_id", "service_id"),
        default_bindings={"tool_hint": "credential_validator"},
    ),
    "check_credential_reuse": ActionTemplate(
        action_type="check_credential_reuse",
        task_type=TaskType.CREDENTIAL_REUSE_VALIDATION,
        label="Check credential reuse",
        estimated_risk=0.6,
        estimated_noise=0.3,
        approval_required=True,
        resource_key_fields=("credential_id", "host_id", "service_id"),
