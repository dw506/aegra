...
**权限边界只剩三条**（其余一律默认放行）：
1. **授权 scope 边界**：由外部 MCP 工具服务或目标适配器强制（工具进程外校验目标，越界即拒）。客户端 executor 的 `policy_check` 仅 audit-only。（历史说明：客户端 `PolicyEngine.evaluate_target_scope` 从未接线到活路径，已删除。）
...
- 安全不变量：外部工具服务/适配器强制授权 scope 边界、secrets 永不进图
...
1. **授权 scope 边界**：由外部 MCP 工具服务或目标适配器强制（工具进程外校验目标，越界即拒）。客户端 `policy_check` 仅 audit-only。（客户端 `PolicyEngine` 硬闸从未接线，已删除。）
