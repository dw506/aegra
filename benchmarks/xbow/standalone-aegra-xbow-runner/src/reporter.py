...
class Reporter:
    ...
    def log_result(self, result: BenchmarkResult):
        ...
        # Print to console
        console_msg = f"  {status_marker} {result.status} ({duration_str}, ${result.cost_usd:.2f})"
    ...
    def _write_text_summary(self, summary: BenchmarkSummary):
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("Aegra Benchmark Results")
        lines.append("=" * 60)
        ...
        if success_results:
            ...
            for r in success_results:
                ...
                lines.append(f"  [OK] {r.benchmark_id} ({m}m {s}s, ${r.cost_usd:.2f})")
            lines.append("")

        if failed_results:
            lines.append("Failure Details:")
            for r in failed_results:
                msg = f"  [FAIL] {r.benchmark_id}: "
                if r.error_message:
                    msg += r.error_message
        ...
        if timeout_results:
            ...
            for r in timeout_results:
                m = int(r.duration_seconds // 60)
                lines.append(f"  [TIMEOUT] {r.benchmark_id}: Timeout after {m}m")
            lines.append("")

        if error_results:
            lines.append("Error Details:")
            for r in error_results:
                lines.append(f"  [ERROR] {r.benchmark_id}: {r.error_message or 'Unknown error'}")
            lines.append("")
