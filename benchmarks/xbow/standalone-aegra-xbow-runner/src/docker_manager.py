...
class DockerManager:
    ...
    def start_benchmark(self, benchmark_path: Path) -> dict:
        ...
        build_command = self._build_command(benchmark_path)
        ...
        # Aegra runs on the host, so use the host-exposed compose port directly.
    ...
    def _build_command(self, benchmark_path: Path) -> list[str]:
        """Return the benchmark build command, with a Windows fallback for missing make."""
        if shutil.which("make"):
            return ["make", "build"]

        benchmark = benchmark_path.name
        flag_hash = hashlib.sha256(benchmark.upper().encode("utf-8")).hexdigest()
